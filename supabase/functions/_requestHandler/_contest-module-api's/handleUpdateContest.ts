...
import { validateContestDetails } from "../../_validation/ValidateContestAllDetails.ts";
...
export async function updateContestDetails(req:Request) {
   ...
   try {
    ...
    if (Object.keys(contestDetails).length === 0) {
        return handleAllErrors({status_code:Http_Status_Codes.BAD_REQUEST,error_message:"Request Body is empty",error_time:new Date()});
    }
    ...
    if(!updatedData||updatedData.length==0){
        return handleAllErrors({status_code:Http_Status_Codes.NOT_FOUND,error_message:"Conestid does not exist or contest is deleted",error_time:new Date()});
    }
    ...
    return handleAllErrors({status_code:Http_Status_Codes.NOT_FOUND,error_message:`Unexpected error ${error}`,error_time:new Date()});
   }
